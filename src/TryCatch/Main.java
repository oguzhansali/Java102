...
public class Main {
    ...
    public static String checkArray(int indis) {//Alınan değer için hata çıktısı oluşturuldu.
        try {
            int number = arr[indis];//Kullanıcıdan alınan değer listenin içerisindemi bakılır.
            return "Listenin " + indis + ". elamanı " + arr[indis] + "dir.";//İçindeyse çıktı oluşturulur.
        } catch (ArrayIndexOutOfBoundsException e) {//Liste dışında ise hata yakalanır.
            System.out.println("--------------------");
